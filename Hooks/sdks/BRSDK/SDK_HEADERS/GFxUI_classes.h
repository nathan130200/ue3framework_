...
public:
	...
	void AddFocusIgnoreKeyS ( struct FString Key );
...
public:
	...
	struct FString LocalizeFromPath ( struct FString inPath );
	struct FString LocalizedToFlash ( struct FString sSource );
	bool eventWidgetUnloaded ( struct FName WidgetName, struct FName WidgetPath, class UGFxObject* Widget );
	bool eventWidgetInitialized ( struct FName WidgetName, struct FName WidgetPath, class UGFxObject* Widget );
	...
	int eventCreateCallback ( class UGFxObject* pObj, struct FName MemberName );
};
...
public:
	...
	{
		if ( ! pClassPointer )
			pClassPointer = (UClass*) UObject::GObjObjects()->Data[ 31890 ];
