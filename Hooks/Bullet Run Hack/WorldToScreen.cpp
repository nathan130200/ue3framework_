...
#include "Framework\UE3\WorldToScreen.h"
#include "Framework\UE3\UE3Math.h"

void WorldToScreen::GetAxis( FRotator Angles, FVector& vRight, FVector& vUp, FVector& vForward )
{
	float        angle;
...
FLOAT WorldToScreen::VectorDotProduct( FVector* pV1, FVector* pV2 )
{
	return ( ( pV1->X * pV2->X ) + ( pV1->Y * pV2->Y ) + ( pV1->Z * pV2->Z ) );
}

FVector* WorldToScreen::VectorSubtract( FVector* pOut, FVector* pV1, FVector* pV2 )
{
	//*pOut = *pV1 - *pV2; 
...
FVector WorldToScreen::World(UCanvas* Canvas, FVector WorldLocation)
{
...
FVector WorldToScreen::BRBones(FVector Location, APawn* Pawn)
{
	FVector vHeadBone;
