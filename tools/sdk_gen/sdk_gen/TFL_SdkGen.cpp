...
void GenerateVirtualFunc ( UClass* pClass )
{
	...
	for ( unsigned long i = 0x0; i < ( PROCESS_EVENT_OFFSET / 0x4 ); i++ )
	{
		// stream to main buffer (virtual functions padding)
		ssStreamBuffer0 << "\tvirtual void VirtualFunction" << SDKMC_SSDEC ( i, 2 ) << " ( );\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t// " << SDKMC_SSHEX ( ( *(unsigned long*) ( VfTable + ( i * 0x4 ) ) ), 8 ) << " (" << SDKMC_SSHEX ( ( i * 0x4 ), 2 ) << ")\n";
	}

	// stream to main buffer (virtual function ProcessEvent)
	ssStreamBuffer0 << "\tvirtual void ProcessEvent ( class UFunction* pFunction, void* pParms, void* pResult = NULL );\t\t\t\t// " << SDKMC_SSHEX ( ( *(unsigned long*) ( VfTable + PROCESS_EVENT_OFFSET ) ), 8 ) << " (" << SDKMC_SSHEX ( PROCESS_EVENT_OFFSET, 2 ) << ")\n";
...
void gethook(UObject* pObject)
{
		...
		char * trans = strstr(str, "Transient.");

		if (trans != NULL)
		{
			char *eng = strstr(trans, "Engine");
